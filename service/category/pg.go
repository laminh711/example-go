...
func (s *pgService) Delete(_ context.Context, p *domain.Category) error {
	...
	errDeleteBooks := s.db.Where("category_id = ?", p.ID).Delete(domain.Book{}).Error
	if errDeleteBooks != nil {
		return errDeleteBooks
	}
	return s.db.Delete(old).Error
}

func (s *pgService) IsNameDuplicate(ctx context.Context, nameToSearch string) (bool, error) {
	var res = &domain.Category{}
	if err := s.db.Find(&res, "name = ?", nameToSearch).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return false, nil
		}
		return true, err
