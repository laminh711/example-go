...
func Test_validationMiddleware_CreateBatch(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Failed cause of duplicate book ID",
			args: args{
				[]domain.Booklend{
					domain.Booklend{
						BookID: existedBook.ID,
						UserID: existedUser.ID,
						From:   time.Date(2018, time.August, 0, 0, 0, 0, 0, time.UTC),
						To:     time.Date(2018, time.August, 8, 0, 0, 0, 0, time.UTC),
					},
					domain.Booklend{
						BookID: existedBook.ID,
						UserID: existedUser.ID,
						From:   time.Date(2018, time.August, 8, 0, 0, 0, 0, time.UTC),
						To:     time.Date(2018, time.August, 16, 0, 0, 0, 0, time.UTC),
					},
				},
			},
